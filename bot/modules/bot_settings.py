...
async def load_config():
    ...
    if len(BASE_URL) == 0:
        ...
        await create_subprocess_shell(f"gunicorn web.wserver:app --bind 0.0.0.0:{BASE_URL_PORT} --worker-class gevent --timeout 900")
...
async def edit_variable(_, message, pre_message, key):
    ...
    if key == 'RSS_DELAY':
        ...
        if config_dict['BASE_URL']:
            await (await create_subprocess_exec("pkill", "-9", "-f", "gunicorn")).wait()
            await create_subprocess_shell(f"gunicorn web.wserver:app --bind 0.0.0.0:{value} --worker-class gevent --timeout 900")
...
async def edit_bot_settings(client, query):
    ...
    if data[1] == 'close':
        ...
        if data[2] in default_values:
            ...
            if config_dict['BASE_URL']:
                await (await create_subprocess_exec("pkill", "-9", "-f", "gunicorn")).wait()
                await create_subprocess_shell("gunicorn web.wserver:app --bind 0.0.0.0:80 --worker-class gevent --timeout 900")
